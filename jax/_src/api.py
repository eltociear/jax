...
def _cpp_jit(
  ...
  def cache_miss(*args, **kwargs):
    ...
    if jax.config.jax_dynamic_shapes:
      ...
      flat_fun = lu.annotate(flat_fun, in_type)
    top_trace = core.find_top_trace(args_flat)
    execute = None
    ...
    fun_, env_trace_todo = core.process_env_traces_call(
        flat_fun, primitive, top_trace and top_trace.level,
        tuple(params.items()))
    tracers = map(top_trace.full_raise, args_flat)
    fun_ = lu.annotate(fun_, flat_fun.in_type)
    if isinstance(top_trace, core.EvalTrace) and not (
      ...
      outs = execute(*args_flat)
    else:
      execute = None
      outs = top_trace.process_call(primitive, fun_, tracers, params)
    out_flat = map(core.full_lower, core.apply_todos(env_trace_todo(), outs))
